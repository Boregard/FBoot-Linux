...
static clock_t t;       //time
static float mytime = 0;//time @ ms
static int foo;
...
#ifdef SHOW_TIME_MS   
   //time @ ms
   printf("Elapsed time: %i ms\n", (int)(((clock()-t)*1000)/CLOCKS_PER_SEC));//time @ ms
#endif

#ifdef SHOW_TIME_S   
   //time @ x.xxx sec
   mytime = (float)((float)(clock()-t)/(float)CLOCKS_PER_SEC);//calculate time
